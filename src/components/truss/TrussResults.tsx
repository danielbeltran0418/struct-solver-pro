  ...
  return (
    <div className="space-y-4">
      <Card title="Desplazamientos nodales">
        <Table headers={["Nodo", "u (m)", "v (m)"]}
               rows={(result.displacements ?? []).map((d, i) => [
                 model.nodes[i].id, fmt(d[0], 6), fmt(d[1], 6),
               ])} />
      </Card>
      <Card title="Reacciones en apoyos">
        <Table headers={["Nodo", "Rx (kN)", "Ry (kN)"]}
               rows={(result.reactions ?? []).map((r, i) => [
                 model.nodes[i].id, fmt(r[0], 3), fmt(r[1], 3),
               ])
               .filter((_, i) => model.nodes[i].fixed_u || model.nodes[i].fixed_v)} />
      </Card>
      <Card title="Fuerzas axiales por barra">
        <Table headers={["Barra", "i", "j", "N (kN)", "Estado"]}
               rows={(result.member_forces ?? []).map((mf) => {
                 const e = model.elements[mf.spanIndex];
                 return [e.id, e.nodeI, e.nodeJ, fmt(mf.N ?? 0, 3),
