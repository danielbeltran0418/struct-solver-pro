...
export function TrussConfig({
...
}: { model: TrussModel; onChange: (m: TrussModel) => void; onSolve: () => void }) {
  const updateNode = (i: number, patch: Partial<TrussNode>) =>
    onChange({ ...model, nodes: model.nodes.map((n, idx) => idx === i ? { ...n, ...patch } : n) });
  const addNode = () => {
    const id = "N" + (model.nodes.length + 1);
    onChange({
      ...model,
      nodes: [...model.nodes, { id, x: 0, y: 0, fixed_u: false, fixed_v: false, fx: 0, fy: 0 }],
    });
  };
  ...
  const updateElement = (i: number, patch: Partial<TrussElement>) =>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="text-slate-500 text-xs">
                ...
                <th className="text-center pb-1" title="Fija X">⊣X</th>
                <th className="text-center pb-1" title="Fija Y">⊥Y</th>
                <th className="text-left pb-1">Fx (kN)</th>
                <th className="text-left pb-1">Fy (kN)</th>
                <th></th>
              </tr>
            </thead>
            <tbody>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead>
              <tr className="text-slate-500 text-xs">
                <th className="text-left pb-1">ID</th>
                <th className="text-left pb-1">Nodo i</th>
                <th className="text-left pb-1">Nodo j</th>
